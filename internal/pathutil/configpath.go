...
func UpdaterDefaults() ConfigPaths {
	paths := make(ConfigPaths)
	if wd, err := os.Getwd(); err == nil {
		paths[WorkingDirLoc] = filepath.Join(wd, "config.yml")
	}
	paths[HomeLoc] = filepath.Join(HomeDir(), ".skycoin/skywire-updater/config.yml")
	paths[LocalLoc] = "/usr/local/skycoin/skywire-updater/config.yml"
	return paths
}
